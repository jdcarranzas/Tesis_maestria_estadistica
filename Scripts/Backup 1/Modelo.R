...
calculate_V_e <- function(Sigma, t){
  ...
  V_e = solve(kronecker(Sigma, diag(t)))
  return(V_e)
}
...
calculate_V_beta_und <- function(tau, H, H_null){
  V_beta_und = solve(kronecker(diag(9), H %*% t(H) + tau * H_null%*%t(H_null)))
  return(V_beta_und)
}
...
sample_b <- function(V, nu, V_und, b_hat){
  V_bar = chol(chol2inv(chol(V_und)) + nu * chol2inv(chol(V))) |> chol2inv()
...
sample_beta <- function(V_beta, nu, V_beta_und, b_hat_beta, y_hat){
  V_beta_bar = chol(chol2inv(chol(V_beta_und)) + nu * chol2inv(chol(V_beta))) |> chol2inv()
...
sample_nu <- function(nu_nu, n, N, r, k, mu_nu, b, V){
  ...
  mu_bar_nu = nu_bar_nu * solve((nu_nu - n * N * r)/mu_nu + t(b) %*% (V) %*% b)
...
N = 3
r = 3
lags <- c(1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4)
...
for (i in 1:length(k)) {
  ...
  .sigma = map(.beta, function(x) (1 / nu) * (chol(t(x) %*% solve(P_tau) %*% x) |> chol2inv()) |> kronecker(diag(4)))
  alpha_list[[i]] = map(.sigma, function(x) mvtnorm::rmvnorm(1, mean = rep(0,12), sigma = x)) |> unlist()
}
...
for (i in 1:length(t)) {
  ...
  V_und_list[[i]]      = calculate_V(x_m_list[[i]], 
                                     V_e_list[[i]], N, n, 
                                     k[i], r)
}
...
for (i in 1:length(k)) {
  ...
  x_hat_list[[i]] = calculate_x_hat(A_list[[i]], y_lvl_list[[(i-1)%/%3 + 1]], N)
...
# 3. MCMC -----------------------------------------------------------------
MCMC_model <- function(n_sams, n_burn, n, N, r, lags, t, k, nu_nu, mu_nu, nu_tau, mu_tau, H_null, x_i_t, y_raw, y_raw_hat, y_lvl, V_und, V_beta_und,
  ...
  # Almacenamiento 
  B_POST <- BETA_POST <- ALPHA_POST <- NU_POST <- TAU_POST <- SIGMA_POST <- LV_POST <- NULL
...
no_cores <- detectCores()-13 # 8 núcleos
...
cadenas <- foreach(i=1:no_cores) %dopar%
  assign(paste0("cadena_", i,"_rezagos_", lags[i]),
         list(MCMC_model(100, 100,
                         n, N, r,
                         lags[i],
                         t[i],
                         k[i],
                         nu_nu, mu_nu, nu_tau, mu_tau, H_null,
                         x_i_t_list[[(i-1)%/%3 + 1]],
                         y_raw_list[[i]],
                         y_raw_hat[[(i-1)%/%3 + 1]],
                         y_lvl_list[[(i-1)%/%3 + 1]],
...
#load("E:/PERSONAL/UN/Maestría Estadística/Tesis/Scripts/Tesis/Cadenas (Finalizado).RData")

# cadenas[[7]][[1]]$ALPHA_POST[,10:20] |> apply(MARGIN = 2, FUN = quantile, c(0.025, 0.975))
# cadenas[[8]][[1]]$ALPHA_POST[,10:20] |> apply(MARGIN = 2, FUN = quantile, c(0.025, 0.975))
# 
# require(doParallel)
# 
...
# beta_ast_list <- list()
# for (i in 1:6) {
#   b_list[[i]]        = rnorm((84), 0, 1)
#   b_list[[i]][seq_along(b_list[[i]])  %% (k[1] + r) %in%  c(1:r)] <- b_aux
#   beta_ast_list[[i]] = rnorm((84), 0, 1)
# }
# 
# set.seed(1234)
# cadenas <- foreach(i=1:6) %dopar%
#   assign(paste0("cadena_", i,"_rezagos_", lags[1]),
#          list(MCMC_model(1000, 100,
#                          n, N, r,
#                          lags[1],
#                          t[1],
#                          k[1],
...
for (i in 1:3) {
  print(cadenas[[i]][[1]]$ALPHA_POST |> apply(MARGIN = 2, FUN = quantile, c(0.025, 0.975)))
}
.a
