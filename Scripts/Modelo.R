...
data <- read_xlsx("../../Documento - Proyecto/data_escalada.xlsx",
                  sheet = "Sheet1") 
...
calculate_V_und <- function(V_und, k, r, Beta, nu, P_tau, n){
  ...
  V_und[seq_along(1:ncol(V_und)) %% (k[1]+r) %in% c(1:r), 
        seq_along(1:ncol(V_und)) %% (k[1]+r) %in% c(1:r)] <- map(Beta, function(x) kronecker((1/nu) * Matrix::solve(t(x) %*% Matrix::solve(P_tau) %*% x), diag(n))) |> 
    GDINA::bdiagMatrix()
  return(V_und)
}
...
sample_b <- function(V, nu, V_und, b_hat){
  V_bar = Matrix::chol(nu * chol2inv(chol(V_und + diag(ncol(V_und))/10000)) + Matrix::chol2inv(Matrix::chol(V))) |> Matrix::chol2inv()
  b_bar = V_bar %*% Matrix::chol2inv(Matrix::chol(V)) %*% b_hat
...
sample_beta <- function(V_beta, nu, V_beta_und, b_hat_beta, y_hat){
  V_beta_bar = Matrix::chol(nu*Matrix::chol2inv(Matrix::chol(V_beta_und)) + Matrix::chol2inv(Matrix::chol(V_beta))) |> Matrix::chol2inv()
...
sample_nu <- function(nu_nu, n, N, r, k, mu_nu, b, V){
  ...
  mu_bar_nu = nu_bar_nu * Matrix::solve((nu_nu - n * N * r)/mu_nu + t(b) %*% inverse_matrix(V + diag(ncol(V))) %*% b)
...
for (i in 1:length(t)) {
  ...
  x_m_list[[i]]        = calculate_x_m(x_i_t_list[[i]], z_i_t_list[[i]], n)
  Sigma_list[[i]]      = Matrix::solve(rWishart::rWishart(n = 1, df = t[i], Sigma = diag(12))[,,1])
  V_e_list[[i]]        = calculate_V_e(Sigma_list[[i]], t[i])
  V_und_list[[i]]      = (rWishart::rWishart(n = 1, df = t[i] + t[i], Sigma = matrix(0.1, 
                                                                                                  nrow = length(b_list[[i]]),
                                                                                                  ncol = length(b_list[[i]])),)[,,1])
...
for (i in 1:length(k)) {
  ...
  x_hat_list[[i]] = calculate_x_hat(A_list[[i]], y_lvl_list[[i]], N)
...
MCMC_model <- function(n_sams, n_burn, n, N, r, lags, t, k, nu_nu, mu_nu, nu_tau, mu_tau, H, H_null, x_i_t, y_raw, y_raw_hat, y_lvl, V_und, V_beta_und,
  ...
  # Almacenamiento 
  B_POST <- BETA_POST <- ALPHA_POST <- NU_POST <- TAU_POST <- SIGMA_POST <- LV_POST <- PI_POST <- ERROR_POST<- NULL
...
no_cores <- 4 # 8 núcleos
cl       <- makeCluster(no_cores, outfile = "") # Opción para clústeres en Windows
...
tictoc::tic()
for (i in 1:1) {
  cadenas[[i]] <- list(MCMC_model(1000, 1000,
                           n, N, r,
                           lags[i],
                           t[i],
                           k[i],
...
coda::effectiveSize(LV_POST |> Reduce(f = 'rbind'))
...
A = matrix(c(-0.23, 0.31, -0.1, -0.2, 0.1, -0.3, -0.1,  0,  0, 0.1, -0.2, 0.14), ncol = 3, byrow = T)
B = matrix(c(0.1, -0.2, 0.2, 0.2, 0.12, -0.1, 0.2, -0.2, 0.03, -0.04, 0.1, -0.1), ncol = 3, byrow = T)
PI = A %*% t(B)
...
par(mfrow = c(6,8), mar=c(1,1,1,0))
for (i in 1:48) {
  comp_pi[,i] |> hist(breaks = 50, main = NULL)
