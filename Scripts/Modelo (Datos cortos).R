...
library(reticulate)
source_python("testeo_inversa.py")
...
data_sim <- read_xlsx("../../Data/sims.xlsx", sheet = "Hoja4") |> dplyr::select(A, B, C, D)
...
calculate_V_e <- function(Sigma, t){
  ...
  V_e = inverse_matrix(kronecker(Sigma, diag(t))) 
  return(V_e)
}

calculate_V_beta <- function(x_m, V_e, N, n, r){
  V = Matrix::chol2inv(Matrix::chol(t(x_m) %*% V_e %*% x_m  + (diag(N * n * r) / 100)))
  return(V)
}

calculate_V <- function(x_m, V_e, N, n, k, r){
  V = inverse_matrix((t(x_m) %*% V_e %*% x_m  + (diag(N * n * (k + r)) / 100)))
  return(V)
}
...
calculate_V_und <- function(V_und, k, r, Beta, nu, P_tau, n){
  ...
  V_und[seq_along(1:ncol(V_und)) %% (k[1]+r) %in% c(1:r), 
        seq_along(1:ncol(V_und)) %% (k[1]+r) %in% c(1:r)] <- map(Beta, 
                                                                 function(x) kronecker((1/nu) * Matrix::solve(t(x) %*% Matrix::solve(P_tau) %*% x), 
                                                                                       diag(n)) |> round(8)) |> GDINA::bdiagMatrix()
  return(V_und)
}
...
# V_und = Matrix::solve(calculate_V(x_m, V_e))
...
sample_sigma <- function(e, t){
  sampled_sigma = Matrix::chol(rWishart::rWishart(n = 1, df = t, Sigma = (calculate_sigma(e)))[,,1]) |> Matrix::chol2inv()
...
sample_b <- function(V, nu, V_und, b_hat){
  V_bar = solve(nu * chol2inv(chol(V_und  + diag(ncol(V))/10000)) + chol2inv(chol(V + diag(ncol(V))/10000)))
  b_bar = V_bar %*% chol2inv(chol(V + diag(ncol(V))/10000)) %*% b_hat
...
sample_beta <- function(V_beta, nu, V_beta_und, b_hat_beta, y_hat){
  V_beta_bar = Matrix::chol(nu*Matrix::chol2inv(Matrix::chol(V_beta_und)) + Matrix::chol2inv(Matrix::chol(V_beta)) + diag(ncol(V_beta))/10000) |> Matrix::chol2inv()
  b_beta_bar = V_beta_bar %*% Matrix::chol2inv(Matrix::chol(V_beta)) %*% b_hat_beta
...
sample_nu <- function(nu_nu, n, N, r, k, mu_nu, b, V){
  ...
  mu_bar_nu = nu_bar_nu * Matrix::solve((nu_nu - n * N * r)/mu_nu + t(b) %*% Matrix::chol2inv(Matrix::chol(V + diag(ncol(V))/10000)) %*% b)
  sampled_nu = rgamma(n = 1, shape = mu_bar_nu, scale = nu_bar_nu)
  sampled_nu_corr = ifelse(sampled_nu <= 1e-100, rgamma(n = 1, shape = nu_nu, scale = mu_nu), sampled_nu)
  return(sampled_nu_corr)
}

# 1.4.7. Muestrear tau --------------------------------------------------------
sample_tau <- function(nu_tau, N, n, r, mu_tau, nu, beta_ast_list, H_null){
  nu_bar_tau = nu_tau + N * (n * r - r^2)
  mu_bar_tau = nu_bar_tau * Matrix::solve(nu_tau / mu_tau + nu * lapply(beta_ast_list, function(x) diag(t(x) %*% H_null %*% t(H_null) %*% x) |> 
                                                                  sum()) |> unlist() |> sum())
...
sample_alpha <- function(z_i_t, Beta_list, nu, P_tau, n, y_raw, t, Sigma){
  ...
  beta_tau_pri = map(Beta_list, function(x) (1 / nu) * (Matrix::chol(t(x) %*% Matrix::chol2inv(Matrix::chol(P_tau)) %*% x) |> Matrix::chol2inv())) |> Reduce(f = "+")
  g_inv_prior = map(Beta_list, function(x) (1 / nu) * (Matrix::chol(t(x) %*% Matrix::chol2inv(Matrix::chol(P_tau)) %*% x) |> Matrix::chol2inv()) |> kronecker(diag(n))) |> Reduce(f = "+")
  y_raw = split(y_raw, rep(1:3, each = t * n)) |> lapply(function(x) matrix(x, ncol = n)) |> GDINA::bdiagMatrix()
  
  omega_alpha = Matrix::chol2inv(Matrix::chol(kronecker(t(b_x) %*% b_x, Matrix::solve(Sigma)) + kronecker(beta_tau_pri, g_inv_prior)))
  mu_alpha    = omega_alpha %*% c(Matrix::solve(Sigma) %*% t(y_raw) %*% b_x)
...
sample_beta_2 <- function(A, Beta_list, nu, P_tau, n, y_lvl, y_hat, t, Sigma){
  ...
  omega_b_ast = Matrix::chol2inv(Matrix::chol(kronecker(t(A_mat) %*% Matrix::solve(Sigma) %*% A_mat, t(x) %*% x) + 
                                kronecker(t(A_mat) %*% Matrix::solve(g_inv_prior) %*% A_mat, 1/nu * Matrix::solve(P_tau))))
  mu_b_ast    = omega_b_ast %*% c(t(x) %*% y_h %*% Matrix::solve(Sigma) %*% (A |> GDINA::bdiagMatrix()))
...
for (i in 1:length(k)) {
  ...
  alpha_sigma[[i]] = map(.beta, function(x) (1 / nu) * (Matrix::chol(t(x) %*% Matrix::chol2inv(Matrix::chol(P_tau)) %*% x) |> Matrix::chol2inv()) |> kronecker(diag(4)))
...
for (i in 1:length(t)) {
  ...
  Sigma_list[[i]]      = Matrix::solve(rWishart::rWishart(n = 1, df = t[i], Sigma = diag(12))[,,1])
  V_e_list[[i]]        = calculate_V_e(Sigma_list[[i]], t[i])
  V_und_list[[i]]      = solve(rWishart::rWishart(n = 1, df = 100, Sigma = solve(diag(length(b_list[[i]]))))[,,1])
  ...
  #                                                                                       !(seq_along(b_list[[i]]) %% (k[i]+r) %in% c(1:r))] |> Matrix::solve()
}
...
# Sigma <- Matrix::solve(rWishart::rWishart(n = 1, df = t, Sigma = diag(12))[,,1])
# 
# # V_e
...
for (i in 1:length(k)) {
  b_hyp_list[[i]] = Matrix::chol2inv(Matrix::chol(t(x_m_list[[i]]) %*% x_m_list[[i]])) %*% t(x_m_list[[i]]) %*% (delta_y |> group_by(channel, variable) |> 
                                                                                                   slice_tail(n = t[i]) |> 
                                                                                                   arrange(channel, variable) |> 
                                                                                                   ungroup() |> select(t_0) |> unlist() |> as.numeric())
  beta_ast_hyp_list[[i]] = Matrix::chol2inv(Matrix::chol(t(x_hat_list[[i]]) %*% x_hat_list[[i]])) %*% t(x_hat_list[[i]]) %*% (calculate_y_hat(delta_y, 
...
for (i in 1:B) {
  ...
  if (i%%ncat == 0) cat(100*round(i/B, 1), "% completado ... \n", sep = "")
  print(i)
  # Almacenamiento (Actualización)
  if(i > n_burn){
    B_POST[[i-n_burn]]      <- b
...
# load("Cadenas (10000 ejecuciones, sim Extrma).RData")


.B_POST  = B_POST |> Reduce(f = 'rbind')
.PI_POST = PI_POST |> Reduce(f = 'rbind')
comp_phi <- .B_POST[,!(seq_along(b_list[[1]]) %% (k[1]+r) %in% c(1:r))]
Phi   = rep(c(0.3, 0.01, -0.13, 0, 0, 0.4, -0.3, 0, 0.3, -0.24,  0.1, -0.3, -0.2, 0.45, -0.09, -0.12),3)

par(mfrow = c(6,8), mar=c(1,1,1,0))
for (i in 1:48) {
  comp_phi[,i] |> hist(breaks = 50, xlim = c(-10, 10))
...
A = matrix(c(-0.23, 0.1, -0.1, -0.07, 0.17, -0.23, -0.1,  0,  0, 0.1, 0.37, 0.14), ncol = 3)
B = matrix(c(0.1, -0.07, 0.18, 0.09, 0.17, -0.1, 0.2, -0.2, 0.03, -0.04, 0.19, -0.1), ncol = 3)

PI   = A%*%t(B) |> c() |> rep(3)

par(mfrow = c(6,8), mar=c(1,1,1,0))
for (i in 1:48) {
  comp_pi[,i] |> hist(breaks = 50, xlim = c(-10, 10))
...
# Tamaños efectivos
neff_pi <- coda::effectiveSize(comp_pi)
neff_phi <- coda::effectiveSize(comp_phi)
neff_nu <- coda::effectiveSize(NU_POST |> unlist())
neff_tau <- coda::effectiveSize(TAU_POST |> unlist())

# Errores estándar
round(apply(X = comp_phi, MARGIN = 2, FUN = sd)/sqrt(neff_phi), 3)
round(apply(X = comp_pi, MARGIN = 2, FUN = sd)/sqrt(neff_pi), 3)
